...
static keyNum_t IN_TranslateSDLToQ3Key( SDL_Keysym *keysym, qboolean down )
{
	...
	if( keysym->scancode >= SDL_SCANCODE_1 && keysym->scancode <= SDL_SCANCODE_0 )
	{
		...
		if( keysym->scancode == SDL_SCANCODE_0 )
			...
			key = (keyNum_t)((int)'1' + (int)keysym->scancode - (int)SDL_SCANCODE_1);
	}
		...
		switch( keysym->sym )
		{
			...
			default:
				if( !( keysym->sym & SDLK_SCANCODE_MASK ) && keysym->scancode <= 95 )
				{
					...
					// to SDL 1.2.
					key = (keyNum_t)((int)K_WORLD_0 + (int)keysym->scancode);
				}
				break;
		}
	}
...
void IN_Frame( void )
{
	...
	cls.glconfig.isFullscreen = (qboolean)(Cvar_VariableIntegerValue( "r_fullscreen" ) != 0);
...
void IN_Init( void *windowData )
{
	...
	IN_DeactivateMouse( (qboolean)(Cvar_VariableIntegerValue( "r_fullscreen" ) != 0) );
