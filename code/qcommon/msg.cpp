...
void MSG_WriteDeltaUsercmdKey( msg_t *msg, int key, usercmd_t *from, usercmd_t *to ) {
	...
	if (from->angles[0] == to->angles[0] &&
		...
		from->interactionButtons == to->interactionButtons &&
	...
	MSG_WriteDeltaKey( msg, key, from->interactionButtons, to->interactionButtons, 16 );
...
void MSG_ReadDeltaUsercmdKey( msg_t *msg, int key, usercmd_t *from, usercmd_t *to ) {
	...
	if ( MSG_ReadBits( msg, 1 ) ) {
		...
		to->interactionButtons = MSG_ReadDeltaKey( msg, key, from->interactionButtons, 16);
		...
		to->interactionButtons = from->interactionButtons;
...
#define CompNetField( x ) #x, (size_t)&((Components::SharedComponent*)0)->x
#define CompNetFieldExplicit( stringname, codename ) stringname, (size_t)&((Components::SharedComponent*)0)->codename
...
//		pcount[i]++;
	...
	for ( i = 0; i < MAX_WEAPONS; i++ )
	{
		if ( to->ammoMag[i] != from->ammoMag[i] )
		{
			ammomagbits |= 1 << i;
		}
	}
	...
	if (!statsbits && !persistantbits && !ammobits && !ammomagbits && !powerupbits) {
		MSG_WriteBits( msg, 0, 1 );	// no change
		oldsize += 4;
		return;
	}
	...
	{
		MSG_WriteBits( msg, 1, 1 ); // changed
		MSG_WriteBits( msg, ammomagbits, MAX_WEAPONS );
		for ( i = 0; i < MAX_WEAPONS; i++ )
			if ( ammomagbits & (1 << i) )
				MSG_WriteShort( msg, to->ammoMag[i] );
	}
	else 
	{
		MSG_WriteBits( msg, 0, 1 );	// no change
	...
	if (MSG_ReadBits( msg, 1 ) ) {
		...
		if ( MSG_ReadBits( msg, 1 ) )
		{
			...
			for ( i = 0; i < MAX_WEAPONS; i++ )
			{
				if ( bits & (1 << i) )
				{
					to->ammoMag[i] = MSG_ReadShort( msg );
				}
			}
		}
