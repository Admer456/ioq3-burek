...
byte *RB_ReadPixels(int x, int y, int width, int height, size_t *offset, int *padlen)
{
	...
	buffer = static_cast<byte*>( ri.Hunk_AllocateTempMemory(padwidth * height + *offset + packAlign - 1) );
	
	bufstart = static_cast<byte*>( PADP((intptr_t) buffer + *offset, packAlign) );
...
void R_TakeScreenshot( int x, int y, int width, int height, char *name, qboolean jpeg ) {
	...
	cmd = static_cast<screenshotCommand_t*>( R_GetCommandBuffer( sizeof( *cmd ) ) );
	if ( !cmd ) {
		return;
	}
...
void R_ExportCubemaps(void)
{
	...
	cmd = static_cast<exportCubemapsCommand_t*>( R_GetCommandBuffer(sizeof(*cmd)) );
	if (!cmd) {
		return;
	}
...
const void *RB_TakeVideoFrameCmd( const void *data )
{
	...
	cBuf = static_cast<byte*>( PADP(cmd->captureBuffer, packAlign) );
...
void R_Register( void ) 
{
	...
	r_baseParallax = ri.Cvar_Get( "r_baseParallax", "0", CVAR_ARCHIVE | CVAR_LATCH );
	r_baseSpecular = ri.Cvar_Get( "r_baseSpecular", "0", CVAR_ARCHIVE | CVAR_LATCH );
	r_baseGloss = ri.Cvar_Get( "r_baseGloss", "1.0", CVAR_ARCHIVE | CVAR_LATCH );
	r_glossType = ri.Cvar_Get("r_glossType", "2", CVAR_ARCHIVE | CVAR_LATCH);
	r_dlightMode = ri.Cvar_Get( "r_dlightMode", "5", CVAR_ARCHIVE | CVAR_LATCH );
	...
	r_textureMode = ri.Cvar_Get( "r_textureMode", "GL_NEAREST_MIPMAP_LINEAR", CVAR_ARCHIVE );
	...
	r_ambientScale = ri.Cvar_Get( "r_ambientScale", "0.23", CVAR_CHEAT );
	...
	r_offsetFactor = ri.Cvar_Get( "r_offsetfactor", "-4", CVAR_CHEAT );
	r_offsetUnits = ri.Cvar_Get( "r_offsetunits", "-128", CVAR_CHEAT );
...
//	Swap_Init();
	...
	ptr = static_cast<byte*>( ri.Hunk_Alloc( sizeof( *backEndData ) + sizeof(srfPoly_t) * max_polys + sizeof(polyVert_t) * max_polyverts, h_low) );
